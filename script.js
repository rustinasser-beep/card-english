(function() {
    ...
    function updateUI() {
        ...
        if (state.currentPage === 'testPage' && state.testSubMode === 'choice') {
    ...
    function checkWriting() {
        ...
        if (!input) {
            elements.testFeedback.textContent = '⚠️ اكتب الإجابة';
    ...
    function processAnswer(isCorrect) {
        ...
        saveState();
        setTimeout(() => {
            if (state.currentPage === 'testPage') navigate(1);
        }, 1200);
    }
    ...
    function parseFile(text) {
        ...
        for (const line of lines) {
            ...
            if (arabic.toLowerCase().includes('nan')) {
                arabic = arabic.replace(/nan/gi, '').trim() || '⚠️';
            }
            if (!english) english = '?';
            result.push({
                english: english || '?',
        ...
        filtered.forEach((word, index) => {
            ...
            item.innerHTML = `
                ...
                <div class="word-actions">
                    <button class="status-btn ${word.status}" data-index="${actualIndex}">
    ...
    function deleteWord(index) {
        if (confirm('هل تريد حذف هذه الكلمة؟')) {
            ...
            if (state.currentIndex >= state.vocabulary.length) {
                state.currentIndex = Math.max(0, state.vocabulary.length - 1);
            }
            saveState();
            renderWordList(elements.searchInput ? elements.searchInput.value : '');
            updateUI();
    ...
    function addWord() {
        ...
        if (!english || !arabic) {
            alert('يرجى إدخال الكلمة والترجمة معًا');
            return;
        }
    ...
    function setupEvents() {
        ...
        // الاتجاه
        elements.enToArBtn.addEventListener('click', () => {
            state.direction = 'en-ar';
            updateDirectionUI();
            updateUI();
            saveState();
        });
        elements.arToEnBtn.addEventListener('click', () => {
        ...
        elements.dueReviewBtn.addEventListener('click', () => {
            if (state.currentFilter === 'due') {
                state.currentFilter = 'all';
                elements.dueReviewBtn.textContent = '📅 مستحق';
            } else {
                ...
                elements.dueReviewBtn.textContent = '📚 الكل';
            }
            applyFilter();
            state.currentIndex = 0;
            updateUI();
        ...
        elements.newArabic.addEventListener('keypress', e => {
        ...
        document.addEventListener('keydown', e => {
            if (e.target.id === 'guessInput' && e.key === 'Enter') {
                ...
                e.preventDefault();
                navigate(-1);
            } else if (e.key === ' ' && state.currentPage === 'studyPage') {
                e.preventDefault();
