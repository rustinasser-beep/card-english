(function() {
    ...
    function setupFeedbackForm() {
        ...
        const alreadySent = localStorage.getItem('feedback_sent');
        if (alreadySent) {
            form.style.display = 'none';
            success.style.display = 'block';
            success.textContent = '✅ لقد أرسلت رأيك مسبقًا. شكراً لك!';
            return;
        }
    ...
    function setupEvents() {
        ...
        elements.flashcard.addEventListener('click', () => {
            if (getActiveWord()) { elements.flashcard.classList.toggle('flipped'); state.isFlipped = !state.isFlipped; }
        });
